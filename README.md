...
* **Sheet Pop-out Button**: Adds a one-click **↗ pop-out button** to the header of Actor and Item sheets, allowing you to detach the window into a separate browser window using Foundry V14's native pop-out functionality — without going through the three-dot menu. The button is hidden automatically when the sheet is already detached. Has no effect on V13. Enabled by default.
