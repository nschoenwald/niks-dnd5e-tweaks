...
* **Prompt for Initiative**: Automatically prompts connected players (or the GM for NPCs) with their initiative roll configuration dialog when their token or combatant is added to an active combat encounter (choices: **For All** (default), For Players, For NPCs, For None). If no owner is connected for a player character, a whispered chat card with a **Roll Initiative** button is sent to the GM and offline owners as a fallback. Excludes summons and legendary action placeholders.
