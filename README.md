...
* **Self Effect Application Prompt**: When an actor uses an ability that has Active Effects targeting "Self" (e.g. Rage, Divine Favor, Mirror Image), whispers a chat card to the actor's owning player and the GM. The card lists each self-targeted effect with its icon and a one-click **Apply** button. After applying an effect, the button transitions to "Effect Applied ✓" and an **Undo** button appears to delete the effect from the actor if needed. Button state syncs across all clients, matching the same apply/undo pattern as the Player Damage Prompt.
