...
* **Auto-Fly NPC Tokens**: When dragging an NPC token onto the canvas, automatically sets its movement mode to flying if the actor's fly speed is at least as large as its walk speed. This saves the GM from having to manually switch the movement mode for flying creatures.
